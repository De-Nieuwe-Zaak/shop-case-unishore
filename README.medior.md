...
2. **TypeScript:** Required throughout — no JavaScript fallback.
...
2. **TypeScript:**
   ...
   - Use the typed hooks (`useAppDispatch`, `useAppSelector`) from `src/app/hooks.ts`.
...
4. **Cart Integration:**
   ...
   - The existing `cartSlice.ts` currently stores `string[]` with a `TODO` comment. Upgrade it to store proper cart items:
     ```ts
     interface CartItem {
       id: number
       name: string
     ...
     ```
   - Update the existing `addProduct` and `removeProduct` actions accordingly.
